...
/// Экран 29/30 «Подтвердите почту» / «Код сброса» — общая логика ячеек кода.
  ...
  Future<void> _onCodeCompleted(String code) async {
    ...
    try {
      if (widget.purpose == VerifyPurpose.register) {
        ...
        // дальше роутер сам уведёт на онбординг/главный экран по смене статуса сессии
      } else {
            ...
            children: [
              ...
              Center(
                child: _resendSecondsLeft > 0
                    ? Text(
                        l10n.resendCodeTimer(_resendSecondsLeft),
                        style: Theme.of(context).textTheme.bodyMedium,
                      )
                    : TextButton(
                        onPressed: _resend,
